use atlas_mir::{
    InsertionPair, JitOptimizationLevel, observe_jit_add_u64, observe_jit_insertion_pairs,
    observe_jit_is_sorted_i64, observe_jit_partition_even_i64, observe_jit_reverse_i64,
    summarize_host_code,
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for optimization in OPTIMIZATION_LEVELS {
        let mut values = [5, -1, 5, 3, -1, 0]
            .into_iter()
            .enumerate()
            .map(|(original_index, key)| InsertionPair {
                key,
                original_index: original_index as u64,
            })
            .collect::<Vec<_>>();
        let observation = observe_jit_insertion_pairs(&mut values, optimization)
            .map_err(|error| format!("MIR insertion observation failed: {error:?}"))?;
        print_shape(
            "insertion_pairs",
            optimization,
            format!("{values:?}"),
