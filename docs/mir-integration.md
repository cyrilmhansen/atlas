...
## Standalone MIR RV64 generator probe

DEC-049 adds a separate probe for MIR's generator. MIR chooses its target from
the architecture on which `mir-gen.c` is compiled; it is not a host-side cross
compiler with a runtime target switch. The script therefore cross-compiles the
pinned MIR core, generator and one scalar probe as a static RV64GC LP64D ELF,
then executes that generator under `qemu-riscv64`.

Inside QEMU, MIR first builds an `add_u64` function, explicitly selects
optimization level 2, publishes RV64 machine code and executes `40 + 2`. The
existing synchronous observer writes the generated bytes to a temporary file
before the MIR context is destroyed. `riscv64-linux-gnu-objdump` independently
finds the generated addition and return. On the current pinned stack the
function span is 16 bytes and its decoded operations begin with
`add a0,a0,a1` and `ret`.

DEC-050 adds a second, separate MIR context for adjacent `is_sorted`. Its
generated RV64 code receives only an element count and calls the private
`guest_load_i64(offset)` import for each adjacent value. The import checks
non-negative aligned offsets against one bounded region and reconstructs each
signed value from explicit little-endian bytes. The wrapper rejects a mismatch
between byte length and element count before invoking generated code.

The empty, singleton, `[-2, 0, 0, 4]` and `[1, 5, 4, 6]` fixtures match the
native/interpreter/host-JIT correction matrix. They produce ten checked loads
and report the first inversion at index 2 for the last case. An invalid
seven-byte singleton span performs no load. The generated span is 128 bytes on
...
correction probes below. The separate DEC-049 RV64 scalar experiment does not
extend this AST lowering or its guest-memory imports.
