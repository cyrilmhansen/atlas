...
- `docs/phase4/k4-m2-priority-source-review.md`: dynamic competitor sources and dependency tradeoffs;
...
cargo run -p atlas -- qualify priority_queue.push --cost time worst 'O(log n)' --condition state.spare_capacity --cost allocation worst none --condition state.spare_capacity
...
Each `--cost METRIC REGIME BOUND` starts one exact schema 0.2 profile group.
Following `--condition ID` arguments form that group's exact condition set.
Multiple cost groups use AND semantics; unknown conditions are rejected rather
than treated as absent knowledge.
