...
- Added parser-equivalent pseudocode and a typed AST for
  `select.minimum.linear`, including explicit optional output for empty input.
- Compiled the exact reviewed AST to a private nine-instruction program with
  validated registers, targets and semantic AST-node links.
- Added one bounded WASM `VisualMachine` retaining current state rather than a
  presentation trace; strict comparison preserves the first minimum on ties.
...
- 52 `atlas` library tests and 22 `atlas-web-wasm` library tests passed.
...
- The compiler deliberately accepts only the reviewed minimum AST shape.
- The common machine does not yet expose writes or swaps; even partition is the
  next roadmap gate.
- Browser timing includes program parsing and machine construction and remains
  non-portable observation only.

## 2026-07-14 - Activate MVP 6 and widen semantic execution

### Result

- Accepted `mvp6-A`, `gen-B` and `presentation-A` as DEC-061 through DEC-063.
- Activated MVP 6 with a bounded private visual machine, build-generated
  bytecode and derived private presentation description as its selected target.
- Made the AST/state execution band span the viewport, allocated more desktop
  width to current state and placed state before pseudocode on mobile.
- Added the non-normative layered performance-model research report for the
  later performance-fingerprint scope.

### Verification

- `scripts/check-web.sh` passed for all three retained MVP 5 reference paths.
- Chrome headless renders were inspected at 390x844, 768x1024, 1440x900 and
  1920x1080; state, pseudocode and controls remained separated and readable.
- `git diff --check` passed.

### Limits

- At this historical checkpoint the slice changed layout only; the subsequent
  generated-minimum checkpoint implements the bytecode, common visual machine
  and first derived presentation.
- Playwright is not installed; direct Chrome headless screenshots were used
  without adding a frontend test dependency.
