...
Inside QEMU, MIR builds an `add_u64` function, explicitly selects optimization
level 2, publishes RV64 machine code and executes `40 + 2`. The existing
synchronous observer writes the generated bytes to a temporary file before the
MIR context is destroyed. `riscv64-linux-gnu-objdump` independently finds the
generated addition and return. On the current pinned stack the function span is
16 bytes and its decoded operations begin with `add a0,a0,a1` and `ret`.
...
This proves the scalar RV64 generator, executable allocator and QEMU execution
path. It does not define an archived machine-code format, connect Atlas guest
memory or imports, test RV64ILP32, or establish a system/fantasy-computer
profile.
