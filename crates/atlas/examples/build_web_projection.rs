...
use atlas::web_projection::to_json;

fn main() -> ExitCode {
    let mut arguments = env::args_os().skip(1);
    let Some(output_path) = arguments.next().map(PathBuf::from) else {
        eprintln!("usage: build_web_projection OUTPUT_PATH SOURCE_COMMIT");
    ...
    match build(&output_path, &source_commit) {
        Ok(()) => ExitCode::SUCCESS,
        Err(error) => {
...
fn build(output_path: &Path, source_commit: &str) -> Result<(), String> {
    let registry =
    ...
    let json = to_json(&registry, &summary, source_commit).map_err(|error| error.to_string())?;
