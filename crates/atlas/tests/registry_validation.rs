...
fn accepts_the_committed_registry() {
    ...
    assert_eq!(registry.problems.len(), 33);
    assert_eq!(registry.algorithms.len(), 41);
    assert_eq!(registry.implementations.len(), 45);
    ...
    let atlas_implementations = registry
        ...
        .collect::<Vec<_>>();
    assert_eq!(atlas_implementations.len(), 29);
    assert!(
...
fn cli_accepts_an_explicit_registry_path() {
    ...
    assert!(String::from_utf8_lossy(&output.stdout).contains("Validated 33 problem(s)"));
}
...
fn cli_rebuilds_a_deterministic_sqlite_index() {
    ...
    assert!(first_stdout.contains("Indexed 121 entities,"));
